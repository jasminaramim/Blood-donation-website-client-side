...
import BlogPage from './BlogPage';
import useRole from '../../../Hoooks/useRole'; // Ensure the correct import
...
const ContentManagementPage = () => {
  ...
  // Prevent rendering if role is still loading
  if (isRoleLoading) {
    return (
  ...
  return (
    <div className="content-management-page">
      {/* Sticky Navbar */}
      <div className="sticky top-0 z-5 lg:z-50 bg-white shadow-md px-4 py-3 flex justify-between items-center">
        <div className="flex items-center">
          <AiOutlineAppstoreAdd className="text-red-500 mr-2" size={24} />
          <h1 className="text-2xl font-bold text-gray-800">Content Management</h1>
        </div>
        <button
          onClick={openModal}
      ...
      {role?.role && (
        <div className="mt-2 text-center">
          <p className="text-gray-700">Your Role: {role.role}</p>
        </div>
      )}
